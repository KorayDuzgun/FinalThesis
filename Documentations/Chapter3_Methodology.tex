...
This thesis uses two NCM variants across its three experiments. Experiments~1 and~2 use the \textbf{absolute residual} NCM:
\begin{equation}
    R_i = |y_i - \hat{y}_i|
    \label{eq:nonconformity}
\end{equation}
where $y_i$ is the true value and $\hat{y}_i$ is the model's point prediction for calibration sample $i$. This choice is deliberate: Experiments~1 and~2 aim to isolate the effect of \emph{calibration set management} (static vs.\ online) on coverage. Using a fixed NCM function ensures that any observed differences in coverage are attributable to the calibration strategy, not to the NCM. If both the calibration strategy and the NCM were changed simultaneously, it would be unclear which factor drives the improvement.

Experiment~3 uses the \textbf{normalized residual} NCM at the segment level ($R_i = |y_i - \hat{y}_i| / \sigma_s$, where $\sigma_s$ is the median absolute deviation of residuals for segment $s$), producing adaptive interval widths that reflect segment-specific prediction difficulty. This is described in Section~\ref{subsubsec:normalized_cp}.
