...
Hyperparameters were optimized using \textbf{Randomized Search Cross-Validation} with 50~random configurations sampled from the search space shown in Table~\ref{tab:hyperparam_search}. Random sampling was chosen over exhaustive grid search because Bergstra and Bengio~\cite{bergstra2012random} showed that, for high-dimensional hyperparameter spaces, random search typically matches or exceeds the validation performance of grid search at a fraction of the computational cost, by exploring more unique values along the most influential dimensions rather than wasting trials on redundant combinations in the less important ones.
...
After hyperparameter selection, the final models were retrained on the full training set (W1--W3: 7,598 route-level trips; 289,331 segment-level records). The quantitative performance of these trained models on the calibration and test periods, the structure of their residuals, and the spatial heterogeneity of segment-level errors are presented and discussed in Chapter~\ref{ch:results} (Section~\ref{sec:baseline_results}).
...
\paragraph{Nonconformity Measure (NCM).}
The central concept in conformal prediction is the \textbf{nonconformity measure} (NCM)—a function that quantifies how ``unusual'' a prediction is compared to observed outcomes. The NCM is the heart of conformal prediction: it determines both the \emph{validity} (coverage guarantee) and the \emph{efficiency} (interval width) of the resulting prediction intervals.

The choice of NCM function directly affects interval efficiency. A simple NCM produces valid but potentially wide intervals, while a well-chosen NCM can achieve the same coverage guarantee with narrower intervals. Common NCM choices for regression include:

\begin{itemize}
    \item \textbf{Absolute residual}: $R_i = |y_i - \hat{y}_i|$ — produces constant-width intervals for all samples.
    \item \textbf{Normalized residual}: $R_i = |y_i - \hat{y}_i| / \sigma_i$ — produces adaptive-width intervals, where $\sigma_i$ estimates the local difficulty of each sample. Harder-to-predict samples receive wider intervals; easier samples receive narrower intervals.
    \item \textbf{Signed residual}: $r_i = y_i - \hat{y}_i$ — enables asymmetric intervals and conformal predictive distributions.
\end{itemize}

This thesis uses two NCM variants across its three experiments. Experiments~1 and~2 use the \textbf{absolute residual} NCM induced by the default split conformal regressor of the \texttt{calibrated\_explanations} library (which internally wraps the \texttt{crepes} conformal predictive system):
\begin{equation}
    R_i = |y_i - \hat{y}_i|
    \label{eq:nonconformity}
\end{equation}
where $y_i$ is the true value and $\hat{y}_i$ is the model's point prediction for calibration sample $i$. In practice, this produces symmetric prediction intervals of the form $[\hat{y}_{\text{new}} - \hat{q},\; \hat{y}_{\text{new}} + \hat{q}]$ via a single quantile of the calibration residual magnitudes. This choice is deliberate: Experiments~1 and~2 aim to isolate the effect of \emph{calibration set management} (static vs.\ online) on coverage. Holding the NCM fixed across both experiments ensures that any observed differences in coverage are attributable to the calibration strategy, not to the NCM. If both the calibration strategy and the NCM were changed simultaneously, it would be unclear which factor drives the improvement.

Experiment~3 uses the \textbf{normalized residual} NCM at the segment level ($\tilde{R}_i = |y_i - \hat{y}_i| / \sigma_{s(i)}$, where $\sigma_{s(i)}$ is the median of absolute residuals for segment $s(i)$), producing adaptive interval widths that reflect segment-specific prediction difficulty. This is described in Section~\ref{subsubsec:normalized_cp}.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/3_8.png}
...
% ------------------------------------------------------------
\subsection{Static Conformal Prediction (Experiment~1)}
\label{subsec:static_cp}
% ------------------------------------------------------------
...
This design creates a natural gradient for measuring how coverage degrades as the temporal distance from the calibration set increases. The primary confidence level used throughout this thesis is \textbf{90\%} ($\alpha = 0.10$), chosen for three reasons. First, 90\% is the most common nominal coverage level in the transit ETA uncertainty literature, which enables direct comparison with prior work. Second, at this level the prediction intervals remain narrow enough to be operationally actionable---a 90-minute trip with a $\pm$25~minute interval is informative for passenger decision-making, whereas the same trip with a $\pm$60~minute interval at 99\% confidence would be too wide to support any practical decision. Third, 90\% provides enough statistical headroom to detect both under- and over-coverage: at 99\% confidence, even severe miscalibration only drops empirical coverage into the mid-80s, which is hard to distinguish from normal sampling variation. Secondary analyses at 80\% and 95\% confirm that the main findings on coverage degradation are robust across confidence regimes rather than an artifact of the specific 90\% target.
...
% ------------------------------------------------------------
\subsection{Online Adaptive Conformal Prediction (Experiment~2)}
\label{subsec:online_cp}
% ------------------------------------------------------------
...
Standard split conformal prediction assigns the same interval width to all predictions, which is suboptimal when different segments have inherently different levels of predictability. \textbf{Normalized conformal prediction}~\cite{papadopoulos2002inductive} addresses this by dividing each calibration residual by a segment-specific difficulty estimate before computing a single global quantile threshold. The resulting interval width is rescaled by each test sample's local difficulty, producing \textbf{adaptive interval widths}:
...
\textbf{Step 1 --- Per-segment difficulty estimate.} For each segment $s$, the difficulty estimate $\sigma_s$ is computed as the median of absolute calibration residuals drawn from that segment:
\begin{equation}
    \sigma_s = \text{median}\!\left(\{|R_i| : i \in \mathcal{C}_s\}\right) + \varepsilon
    \label{eq:segment_sigma}
\end{equation}
where $\mathcal{C}_s$ is the set of calibration samples belonging to segment $s$, $R_i = y_i - \hat{y}_i$ is the signed residual for sample $i$, and $\varepsilon = 10^{-6}$ is a small constant added for numerical stability (to avoid division by zero on very-low-variance segments). Segments with fewer than 30~calibration samples fall back to a global median-absolute-residual estimate.

\textbf{Step 2 --- Normalized nonconformity scores.} Each calibration residual is normalized by its segment's difficulty estimate, producing a dimensionless score:
\begin{equation}
    \tilde{R}_i = \frac{|R_i|}{\sigma_{s(i)}}
    \label{eq:normalized_score}
\end{equation}
where $s(i)$ denotes the segment to which calibration sample $i$ belongs. Intuitively, $\tilde{R}_i$ measures how unusual sample $i$'s error is \emph{relative to other samples in its own segment}, placing all segments on a common scale regardless of their absolute difficulty.

\textbf{Step 3 --- Global normalized quantile.} A \emph{single} quantile threshold is computed over the normalized scores from all $n$ calibration samples:
\begin{equation}
    \hat{q} = \text{Quantile}\!\left(\{\tilde{R}_i\}_{i=1}^{n},\; \frac{\lceil(n+1)(1-\alpha)\rceil}{n}\right)
    \label{eq:normalized_quantile}
\end{equation}
Using a single global quantile (rather than one per segment) is deliberate: it preserves the finite-sample coverage guarantee of split conformal prediction, because the quantile is always estimated from the full calibration set and is therefore insensitive to rare segments with very few samples.

\textbf{Step 4 --- Adaptive prediction interval.} Finally, the prediction interval for a test sample in segment $s$ is formed by scaling the global threshold by the segment's own difficulty estimate:
\begin{equation}
    C_s(x) = [\hat{y} - \hat{q}\cdot\sigma_s,\; \hat{y} + \hat{q}\cdot\sigma_s]
    \label{eq:segment_interval}
\end{equation}

This construction produces interval widths that are proportional to each segment's historical difficulty (via $\sigma_s$) while preserving the split conformal coverage guarantee (via the global quantile $\hat{q}$).
...
Therefore, the route-level coverage is guaranteed to be at least $1-\alpha$. However, with 30--51 segments per trip (mean $\approx$38), the per-segment confidence becomes very high, ranging from $1 - 0.10/30 \approx 99.67\%$ to $1 - 0.10/51 \approx 99.80\%$, and resulting in very wide segment-level intervals. This conservatism is the main drawback of the Bonferroni approach.
...
% ============================================================
\section{Methodological Design Choices and Limitations}
\label{sec:methodology_limitations}
% ============================================================

Several of the design choices described in this chapter are deliberate scope restrictions rather than oversights, and are therefore best understood as limitations of the methodology itself. Four deserve explicit acknowledgment here, with a fuller discussion of their implications deferred to Chapter~\ref{ch:discussion} (Section~\ref{sec:limitations}).

\textbf{Single baseline model.} The point-prediction stage uses XGBoost exclusively. Deep-learning alternatives such as LSTM, GRU, or Transformer models are not compared. XGBoost was chosen for its strong baseline accuracy, computational efficiency, and tolerance of mixed feature types, all of which make it a practical wrapper target for conformal prediction. However, the findings on coverage degradation may not directly transfer to models whose residual structures are qualitatively different (e.g., recurrent models that explicitly propagate temporal state).

\textbf{Fixed nonconformity measure in Experiments 1 and 2.} Both the static and online experiments use the absolute residual NCM throughout. This is a deliberate experimental control: holding the NCM fixed isolates the effect of calibration set management from the effect of NCM choice. A consequence is that the online coverage improvements reported in Experiment~2 should be interpreted as a lower bound---combining online calibration with an adaptive NCM (such as the normalized NCM used in Experiment~3) would likely yield both better coverage and narrower intervals, but this combination was not empirically evaluated.

\textbf{Two-fold temporal cross-validation.} The forward-chaining CV used for hyperparameter search has only two folds (W1$\rightarrow$W2 and W1--W2$\rightarrow$W3), which is the maximum number of non-overlapping folds obtainable from a three-week training window while respecting temporal ordering. More folds would require either borrowing weeks from the calibration or test periods (which would compromise the conformal prediction experiments downstream) or collecting a longer training window. The two-fold constraint limits the variance control on the hyperparameter search, and should be kept in mind when interpreting the reported generalization gap.

\textbf{Single city and 55-day observation window.} The dataset covers three bus routes in Astana, Kazakhstan, over 55~days from late July to late September~2024. This window spans late summer to early autumn and does not include winter conditions, school holidays, or major infrastructure changes. The generalizability of the findings to other cities, climates, and seasons has not been empirically tested. These constraints are not methodological flaws but practical scope decisions, and they frame the external validity of the results reported in Chapter~\ref{ch:results}.